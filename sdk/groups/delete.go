...
func deleteAPI(connection *common.Connection, groupId string) (*[]DeleteResult, *common.ErrorHUE, error) {
	...
	if err != nil {
		log.Errorf("Error with requesting DELETE on /api/groups/%s (update a group): %s", groupId, err.Error())
		return &[]DeleteResult{}, errHUE, err
	}
	...
	if err != nil {
		log.Errorf("Error with unmarshalling DELETE on /api/groups/%s (update a group): %s", groupId, err.Error())
